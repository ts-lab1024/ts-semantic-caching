...
func (e *DataSourceHandler) QueryData(ctx context.Context, req *backend.QueryDataRequest) (*backend.QueryDataResponse, error) {
	...
	for _, query := range req.Queries {
		...
		//go e.executeQueryCLI(query, &wg, ctx, ch, queryjson)
	}

	wg.Wait()

	atomic.AddInt64(&timescaledb_client.TotalLatency, time.Since(now).Nanoseconds())
	atomic.AddInt64(&timescaledb_client.TotalCount, 1)

	if atomic.LoadInt64(&timescaledb_client.TotalCount)%1 == 0 {
		tcnt := atomic.LoadInt64(&timescaledb_client.TotalCount)
		tltc := atomic.LoadInt64(&timescaledb_client.TotalLatency)

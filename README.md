...
func TestExample(t *testing.T) {
    ...
    queryString := `SELECT mean("usage_nice") FROM "cpu" WHERE ("hostname"='host_0' OR "hostname"='host_1') AND TIME >= '2022-06-01T00:00:00Z' AND TIME < '2022-06-01T11:00:00Z' GROUP BY "hostname",time(1m)`

    qry := NewQuery(queryString, dbName, "s")
    response, err := influxdbConn.Query(qry)
    if err != nil {
        panic(err)
    }

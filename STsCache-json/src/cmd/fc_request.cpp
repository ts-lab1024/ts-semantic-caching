...
{
  uint8_t *data= (uint8_t*)malloc(sizeof(uint8_t)*msg->total_byte_length);
  mbuf_copy_to(&msg->mhdr, msg->value, data, msg->total_byte_length);
  semantic_json::SemanticMetaValue semMeta;
  std::string json_str(reinterpret_cast<const char*>(data), msg->total_byte_length);
  deserialize(json_str, semMeta);

  msg->number_of_table = semMeta.series_array.size();
  for (int i = 0; i < semMeta.series_array.size(); i++) {

    size_t size = 0;
    for (const auto& sample : semMeta.series_array[i]->values) {
      size += sizeof(sample->timestamp);
      size += sample->value.size() * sizeof(double);
    }

    msg->values[i] = (uint8_t *)malloc(size*sizeof(uint8_t));
    uint8_t * ptr = msg->values[i];
    for (const auto& sample : semMeta.series_array[i]->values) {
      uint64_t time = sample->timestamp;
      memcpy(ptr, (uint8_t *)(&time), sizeof(sample->timestamp));
      ptr += sizeof(sample->timestamp);

      for (const auto& v : sample->value) {
        memcpy(ptr, (uint8_t*)(&v), sizeof(v));
        ptr += sizeof(v);
      }
    }

    msg->table_byte_length[i] = ptr-msg->values[i];
  }

    std::string semantic_segment = std::string(reinterpret_cast<const char*>(msg->semantic_segment_start), msg->semantic_segment_len);
    auto key = semantic_graph.SplitSegment(semantic_segment)->second;
    if(msg->number_of_table == 1){
        bool set = SemanticCache::SetProcess(slab_m,semantic_graph,key,semMeta.series_array[0]->series_segment,msg->start_time,msg->end_time,msg->values[0],msg->table_byte_length[0]);
        free(msg->values[0]);
        if (set) {
        ...
        for (uint64_t i = 0; i < msg->number_of_table; i++) {
          bool set = SemanticCache::SetProcess(slab_m,semantic_graph,key,semMeta.series_array[i]->series_segment,msg->start_time,msg->end_time,msg->values[i],msg->table_byte_length[i]);

...
req_process_set(struct context *ctx, struct conn *conn, struct msg *msg)
{
  ...
  SemanticMetaValue semMeta;
  semMeta.ParseFromArray(data, msg->total_byte_length);

  semMeta.series_array(0).values(0).value(0);

  msg->number_of_table = semMeta.series_array_size();
  for (int i = 0; i < semMeta.series_array_size(); i++) {

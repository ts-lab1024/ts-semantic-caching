...
void rsp_send_value(struct context *ctx, struct conn *conn, struct msg *msg, std::vector<uint8_t *>& items,std::vector<std::string> &segments,std::vector<uint32_t > &items_len,std::vector<uint32_t>&per_segment_items_num,std::vector<TimeRange>&per_segment_time_range){
  ...
  for(uint32_t i=0;i<segments.size();i++){
    SemanticSeriesValue semSeries;
    semSeries.set_series_segment(segments[i]);
    ...
    for(uint32_t j=0;j<per_segment_items_num[i];j++){
      ...
      for (int k = 0; k < row_num; k++) {
        ...
        for (int m = 0; m < field_num+1; m++) {
          if (m == 0) {
            smp.set_timestamp(*(uint64_t*)(&items[idx+j][k*(field_num+1)*8]));
          } else {
            smp.add_value(*(double*)(&items[idx+j][(k*(field_num+1)+m)*8]));
          }
        }
        semSeries.add_values()->CopyFrom(smp);
      }

    }
    idx += per_segment_items_num[i];

    semMeta.add_series_array()->CopyFrom(semSeries);
  }

  semMeta.set_semantic_meta(segments[0]);


  size_t serialized_size = semMeta.ByteSizeLong();
  std::vector<uint8_t> buffer(serialized_size);
  semMeta.SerializeToArray(buffer.data(), buffer.size());
  status = mbuf_copy_from(&pmsg->mhdr, buffer.data(), buffer.size());
  pmsg->mlen += serialized_size;
  if (status != FC_OK) {
    req_process_error(ctx, conn, msg, errno);
    return;
  }

...
#define MEMCACHE_MAX_KEY_LENGTH 409600
...
//            }
            ...
            if (r->value == NULL) {
              r->value = p+8;
              r->number_of_table = *(uint64_t *)p-8;
              r->total_byte_length = r->number_of_table;
              p = p + 8;
            }

            m = p + r->number_of_table;
            if (m >= b->last) {
                r->number_of_table -= (uint64_t)(b->last - p);
                m = b->last - 1;
                p = m;

...
func FieldsAndAggregation(queryString string, measurementName string) (string, string) {
	...
	var aggr string
	FGstr = strings.ReplaceAll(FGstr, "\"", "")
...
func GetWriteSemanticKey(queryString string) string {
	semanticSegment := GetReadSemanticKey(queryString)
	partialSegment, _, metric := SplitPartialSegment(semanticSegment)
	writeSemanticKey := GetStarSegment(metric, partialSegment)
	return writeSemanticKey
}

func GetTimeRange(queryString string) (int64, int64) {
	matchStr := `(?i).+WHERE(.+)`
	conditionExpr := regexp.MustCompile(matchStr)
	if ok, _ := regexp.MatchString(matchStr, queryString); !ok {
		return -1, -1
...
func GetReadSemanticKey(queryString string) string {
	readSemanticKey := GetSemanticSegment(queryString)
	return readSemanticKey
}

func CustomSerialization(resp *Response, queryString string) []byte {
	values := make([]byte, 0)
	_, _, _, tags := GetQueryTemplate(queryString)
	semanticSegment := GetReadSemanticKey(queryString)
	...
	values = ResponseToByteArrayWithParams(resp, datatypes, tags, metric, partialSegment)
	return values
}

func CustomDeSerialization(value []byte, readSemanticKey string) (*Response, int, []uint8, [][]int64, [][]string) {
	parts := strings.Split(readSemanticKey, "#")
	fields := "time[int64]," + parts[1]
	datatypes := GetDataTypeArrayFromSF(fields)
	resp, flagNum, flagArr, timeRangeArr, tagArr := ByteArrayToResponseWithDatatype(value, datatypes)
